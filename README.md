...
`data/exports/` holds every crime assigned to its 2021 LSOA, as a long panel of LSOA × month × category × outcome (joins straight onto census and deprivation data), plus an LSOA lookup and a codebook. It's regenerated by `node scripts/export-panel.mjs`, automatically whenever a new police.uk month lands.
...
| `scripts/export-panel.mjs` | research export: crimes → 2021 LSOAs, panel CSV + codebook in `data/exports/` |
